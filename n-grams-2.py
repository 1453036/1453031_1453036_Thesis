...
class glbClass: #singleton class, contains global properties , just for encapsulation
    ...
    def saveToFile(self):
        ...
        outFile = open("./" + self.__resultDir + "sparse-matrix-unl.txt", 'w')
        print "Saving matrix..."
        for i in range(0, len(self.sparseMatrix)):
            ...
            if (i == self.__dataOffset[0]):
                print "Change to label file"
                outFile.close()
                outFile = open("./" + self.__resultDir + "sparse-matrix-lal.txt", 'w')
        outFile.close()
        ...
        # self.target = numpy.array([],  dtype = str) 
        ...
        #     dataFileName = self.__resultDir + self.dataFileNames[currentOffset]
            ...
            #         standardizedRecord = standardizedRecord.tolist()
        ...
        attrListFile.close()
        outFile = open('./result/target.txt', 'w')
